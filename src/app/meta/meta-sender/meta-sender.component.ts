...
const metacoin_artifacts = require('../../../../build/contracts/SupplyChain.json');
...
export class MetaSenderComponent implements OnInit {
  ...
  async register() {
    ...
    this.setStatus('Initiating transaction... (please wait)');
    try {
      const deployedfarmer_contract = await this.MetaCoin.deployed();
      console.log(`hello`);
      const transaction = await deployedfarmer_contract.addFarmer.sendTransaction(person, {from: person});
      if (!transaction) {
        this.setStatus('Transaction failed!');
      } else {
        this.setStatus('Transaction complete!');
  ...
  async is_registered() {
    ...
    try {
      const deployedfarmer_contract = await this.MetaCoin.deployed();
      console.log(`hello`);
      const transaction = await deployedfarmer_contract.isFarmer.sendTransaction(person, {from: person});
      if (!transaction) {
        this.setStatus('Transaction failed!');
      } else {
        this.setStatus('Transaction complete!');
